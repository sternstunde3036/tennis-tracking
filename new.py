...
def calculate_velocity(coords, t):
    """计算速度并处理None值"""
    Vx, Vy, V = [], [], []
    for i in range(len(coords)):
        if i == 0 or i == len(coords)-1:
            Vx.append(None)  # 首尾帧速度无法计算
            Vy.append(None)
            V.append(None)
            continue
        ...
        # 跳过无效帧
        if None in [p_prev, p_curr, p_next] or t_prev == t_next:
            Vx.append(None)
            Vy.append(None)
            V.append(None)
            continue
        ...
        # 中心差分法计算速度
        vx = (p_next[0] - p_prev[0]) / (t_next - t_prev)
        vy = (p_next[1] - p_prev[1]) / (t_next - t_prev)
        v = (vx**2 + vy**2)**0.5
...
# 导出CSV（与视频输出同目录）
output_csv_path = os.path.join(os.path.dirname(output_video_path), "tracking_results.csv")
with open(output_csv_path, 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['frame', 'x', 'y', 'vx (px/s)', 'vy (px/s)', 'v (px/s)', 'timestamp'])
    
    # 计算速度
    Vx, Vy, V = calculate_velocity(coords, t)
    
    # 写入数据
    for i in range(len(coords)):
        row = [
            i,
            coords[i][0] if coords[i] else None,
            coords[i][1] if coords[i] else None,
            Vx[i],
            Vy[i],
            V[i],
            t[i]  # 时间戳
...
print(f"轨迹和速度数据已保存至: {output_csv_path}")
...
# output_csv_path = os.path.join("VideoOutput", "velocity_data.csv")

# with open(output_csv_path, 'w', newline='') as csvfile:
#     writer = csv.writer(csvfile)
#     writer.writerow(['frame', 'vx', 'vy', 'v'])  # 表头
#     for i in range(len(frames)):
#         writer.writerow([frames[i], Vx[i], Vy[i], V[i]])
